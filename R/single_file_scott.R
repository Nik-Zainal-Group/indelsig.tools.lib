...
#' @export
prepare_indels <- function(indels,
                           sampleID,
                           genome.v,
                           verbose=TRUE){
  ...
  if(is.character(indels)==TRUE){
    ...
    stop('[error prepare_indels] Provide either a path to a vcf file or a tab file with chr, position, REF, ALT')
  }
  if(verbose) message("[info prepare_indels] Indel-specific preps")
...
#' @return Extended indel list with 5' and 3' sequence information
#' @export
segment_indels <- function(df,verbose=TRUE){
  ...
  if(verbose) message("[info segment_indels] Running segmentation")
